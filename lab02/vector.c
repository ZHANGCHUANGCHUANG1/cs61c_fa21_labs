...
vector_t *bad_vector_new() {
    ...
    vector_t *retval, v; // v is allocate at stack mem, the mem will release after fucntion return
    retval = &v;
...
vector_t also_bad_vector_new() {
    /* Create the vector */
    vector_t v; //! v is allocate at stack mem, stackoverflow warning
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    if (loc >= v->size)
    {
        tvp = realloc(v->data, sizeof(int)*(loc+1));
        if (tvp == NULL)
        {
            allocation_failed();
        }
        ...
        {
            v->size ++;
            v->data[v->size - 1] = 0;  //(size - 1) points to the last element in data.
        }
